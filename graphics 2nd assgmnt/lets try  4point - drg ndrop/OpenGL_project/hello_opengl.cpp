...
vec2 points5s[5] = { { 325, 260 },{ 420,400 },{ 580,375 },{ 740,275 }, { 740,100 } };
int array[5] = {0,0,0,0,0};
int array5s[5] = { 0,0,0,0,0 };
...
void drawBezierWithBernstein() {
	...
	glBegin(GL_POINTS);
	for (int i = 0; i < 4; i++) {
		if (array[i] == 1) { glColor3f(0.0, 1.0, 0.0); }
		else
		{
...
void drawBezierWithBernstein5s() {
	...
	glBegin(GL_POINTS);
	for (int i = 0; i < 6; i++) {
		if (array5s[i] == 1) { glColor3f(0.0, 1.0, 0.0); }
		else
		{
			glColor3f(1.0, 0.0, 0.0);
		}
...
void processMouse(GLint button, GLint action, GLint xMouse, GLint yMouse)
{
	...
	/*GLint z;
	if((z = getActivePoint1(points, 4, 8, xMouse, winHeight - yMouse)) != -1) {
		color *= -1;

	}

	if ((z = getActivePoint2(points5s, 5, 10, xMouse, winHeight - yMouse)) != -1) {
		color5s *= -1;
	}
	*/
...
{
	/*if (xMouse == points[0].x && winHeight - yMouse == points[0].y) {
...
void processPassiveMotionFunc(int xMouse, int yMouse) {
	GLint z;
	if ((z = getActivePoint1(points, 4, 8, xMouse, winHeight - yMouse)) != -1) {
		array[z] = 1;
	...
	GLint y;
	if ((y = getActivePoint2(points5s, 5, 10, xMouse, winHeight - yMouse)) != -1) {
		array5s[y] = 1;
...
int main(int argc, char** argv)
{
	...
	glutPassiveMotionFunc(processPassiveMotionFunc);
	glutMainLoop();
	return 0;
}
